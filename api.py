...
================================================
This revision focuses on DIAGNOSABILITY, not on forcing "correct" outputs.
No rule in this file guarantees a specific emotion class will be predicted
correctly — that depends on the trained model's actual accuracy (~61%
validation, actor-independent, per the training pipeline). What this file
DOES guarantee:
  - The encoder label order is validated at startup, not assumed.
  - Every fusion/context decision is logged with WHY it fired.
  - No silent heuristic can flip a prediction without leaving a trace in
    the response's `diagnostics` block.

Upgrades applied (carried over):
  1. Six emotion classes: Angry, Disgust, Fear, Happy, Neutral, Sad
  2. MediaPipe Face Mesh (falls back to Haar cascade)
  3. Whisper-base transcription
  4. Energy-gate silence detection
  5. Synchronous NLP (no race conditions / stale globals)
  6. Micro-expression optical flow on eyes+mouth ROI
  7. Sarcasm detection (face/audio vs text contradiction)
  8. DistilRoBERTa 6-class label mapping
  9. Dynamic fusion weights across AV / FER / Text
  10. Audio preprocessing mirrors training exactly (no denoising drift)
  11. Temporal bbox consistency for optical flow
  12. NEW: startup encoder-order validation (catches stale/mismatched pickles)
  13. NEW: evidence-gated, logged Neutral dampening (was silent before)
  14. NEW: permanent structured diagnostic logging (not a throwaway block)
"""
...
# Expected class order — MUST match the LabelEncoder used at training time.
# sklearn's LabelEncoder sorts alphabetically, so this is what training
# produced. If encoder.pkl doesn't match this at startup, something is
# stale and every downstream prediction is suspect.
...
# =====================================================================
# PYTORCH QUAD-MODAL ARCHITECTURE (MOBILENETV2) — unchanged from training
# =====================================================================
...
# ──────────────────────────────────────────────
# 2. STARTUP — LOAD ALL MODELS + VALIDATE ENCODER
# ──────────────────────────────────────────────
def validate_encoder(enc):
    """
    Confirms the loaded LabelEncoder's class order matches what training
    actually produced. A mismatch here silently corrupts every prediction
    downstream — probs[i] would be attached to the wrong class name — and
    LOOKS like model bias (e.g. "always predicts Fear/Neutral") without
    actually being one. This check exists so that failure mode is caught
    at startup instead of debugged blind, request by request.
    """
    classes = list(enc.classes_)
    print(f"[STARTUP][ENCODER] Loaded classes (in index order): {classes}")
    if classes != EXPECTED_CLASSES:
        print("=" * 70)
        print("[STARTUP][ENCODER] ⚠️  WARNING: encoder.pkl class order does")
        print(f"    NOT match the expected training order: {EXPECTED_CLASSES}")
        print("    Every prediction's label may be misattributed to the")
        print("    wrong probability index. Re-generate encoder.pkl from")
        print("    the exact training run that produced the loaded .pt file.")
        print("=" * 70)
        return False
    print("[STARTUP][ENCODER] ✅ Class order matches expected training order.")
    return True
...
def load_resources():
    ...
    if os.path.exists(os.path.join(MODEL_DIR, "encoder.pkl")):
        ...
        print("[STARTUP][ENCODER] ⚠️  No encoder.pkl found at all — /predict will fail.")

    # No z-score normalisation: training never applied it, only clipping.
...
# ──────────────────────────────────────────────
# 3. AUDIO PREPROCESSING — mirrors training exactly
# ──────────────────────────────────────────────
...
def detect_sarcasm(face_em, audio_em, text_em, face_conf, audio_conf, text_conf):
    """
    Sarcasm = what you SAY (text) contradicts how you LOOK (face) / SOUND (audio).
    Text confidence gate first: no reliable text, no sarcasm call.
    """
    ...
    # Case 1: face + audio agree, text says the opposite — strongest signal
    if face_pos == audio_pos and face_pos != text_pos:
        if face_conf > 0.40 and audio_conf > 0.35:
            return True

    # Case 2: face strongly contradicts text (face must be a specific emotion)
    if face_pos != text_pos and face_conf > 0.50 and text_conf > 0.40 and face_em != "Neutral":
        return True

    # Case 3: audio tone strongly contradicts text
    if audio_pos != text_pos and audio_conf > 0.50 and text_conf > 0.40 and audio_em != "Neutral":
        return True
...
def context_engine(probs_av, probs_fer, probs_text, has_fer, has_text, aud_silent):
    if not (has_fer and has_text and not aud_silent):
        return None, "partial_modalities"
...
def fuse_modalities(probs_av, probs_fer, probs_text, aud_silent, vid_active, motion_mean, brightness, mouth_variance):
    ...
    if has_fer and motion_mean > 2.5:
        wf, wav, wt = 0.80, 0.15, (0.05 if has_text else 0.0)
    if has_fer and brightness < 70:
        wf, wav, wt = 0.65, 0.25, (0.10 if has_text else 0.0)
    if not has_text:
        wt = 0.0
    ...
    if has_fer and not aud_silent:
        ...
        if sarcasm:
            wf, wav, wt = 0.50, 0.40, (0.10 if has_text else 0.0)
    ...
    # ── Evidence-gated Neutral dampening ──
    # Neutral had the best per-class F1 in validation, which also means it's
    # the model's most common "safe default" when uncertain. This ONLY
    # fires when there's a genuinely competitive runner-up AND real motion
    # was observed (a static/uncertain read should NOT be overridden).
    # Every activation is logged so it can be audited, not silently trusted.
    neutral_dampened = False
    ...
    if np.argmax(final) == n_idx:
        ...
        runner_em = encoder.classes_[runner_up]
        if final[runner_up] > 0.30 and motion_mean > 0.5:
            final[n_idx] *= 0.5
            final /= final.sum()
            neutral_dampened = True

    weights = {"av": float(wav), "fer": float(wf), "text": float(wt)}

    # ── Permanent structured diagnostic log ──
    def fmt(probs):
        return ", ".join(f"{encoder.classes_[i]}:{p:.2f}" for i, p in enumerate(probs))

    print(
        f"[FUSION] av=[{fmt(probs_av)}] "
        f"fer=[{fmt(probs_fer) if has_fer else 'N/A'}] "
        f"text=[{fmt(probs_text) if has_text else 'N/A'}] "
        f"weights=(av={wav:.2f},fer={wf:.2f},text={wt:.2f}) "
        f"sarcasm={sarcasm} neutral_dampened={neutral_dampened} "
        f"final=[{fmt(final)}]"
    )

    return final, weights, bool(sarcasm), neutral_dampened
...
def process_prediction_task(task_id, temp_dir, video_path, audio_path):
    try:
        ...
        final_probs, weights, sarcasm, neutral_dampened = fuse_modalities(
            probs_av, probs_fer, probs_text, aud_silent, vid_active, motion_mean, brightness, mouth_var
        )
        ...
        prediction_tasks[task_id] = {
            ...
            "result": {
                ...
                "diagnostics": {
                    ...
                    "neutral_dampened": neutral_dampened,
                },
            },
        }
...
def health_check():
    return {
        ...
        "encoder_order_valid": list(encoder.classes_) == EXPECTED_CLASSES if encoder else False,
    }
